...
**timerRunnableNeeded**:控制是否启用用于获取定时器作业的可运行任务（Runnable）。
...
**maxAsyncJobsDuePerAcquisition**：设置每次获取的异步作业或历史作业的最大数量。
...
```
**defaultTimerJobAcquireWaitTime**:设置定时器作业获取线程在每次执行获取逻辑之间的等待时间。
...
**defaultAsyncJobAcquireWaitTime**:设置异步作业获取线程在每次执行获取逻辑之间的等待时间
...
```
**resetExpiredJobsInterval**:设置重置过期作业的线程在执行下一次重置逻辑前的等待时间。
...
**resetExpiredJobsPageSize**:设置在一次重置周期中，重置过期作业的最大数量。
